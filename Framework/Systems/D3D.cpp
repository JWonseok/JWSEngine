...
ID3D11DeviceContext* D3D::context = NULL;
...
void D3D::Present()
{
	swapChain->Present(d3dDesc.bVsync ? 1 : 0, 0);
}
...
void D3D::CreateBackBuffer(float width, float height)
{
	...
	// GetBackBuffer And Create RTV
	{
		...
		hr = D3D::GetDevice()->CreateRenderTargetView(backBuffer, NULL, &renderTargetView);
		Check(hr);
